...
class UnpackerTest {
    ...
    fun testUnpackSimplePackedScript() {
        ...
        val packed = "}('test0 test1',2,2,'word0|word1'.split('|'),0,{}))"
    ...
    fun testUnpackWithRadix62() {
        ...
        val result = Unpacker.unpack(packed)
        assertEquals("zero one ten eleven thirtysix thirtyseven", result)
    }
    ...
    fun testUnpackWithMixedRadix() {
        ...
        val packed = "}('a b c d e f g h i j',36,10,'val0|val1|val2|val3|val4|val5|val6|val7|val8|val9'.split('|'),0,{}))"
        val result = Unpacker.unpack(packed)
    ...
    fun testUnpackWithSubstringExtractor() {
        ...
        val script = "prefix}('0 1',2,2,'word0|word1'.split('|'),0,{})suffix"
    ...
    fun testUnpackWithLargeRadix() {
        ...
        val packed = "}('Z',62,36,'a|b|c|d|e|f|g|h|i|j|k|l|m|n|o|p|q|r|s|t|u|v|w|x|y|z|A|B|C|D|E|F|G|H|I|J'.split('|'),0,{}))"
        val result = Unpacker.unpack(packed)
        assertEquals("J", result) // Z in base62 = 35 + 26 = 61, but we only have 36 items (0-35), so index 61 is out of bounds
    }
