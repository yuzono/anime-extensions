...
class AnimeID : ParsedAnimeHttpSource(), ConfigurableAnimeSource {
    ...
    // Extractores usando el client estándar de Tachiyomi
    ...
    override fun videoListParse(response: Response): List<Video> {
        ...
        // Obtener servidores vía AJAX (data-encrypt)
        ...
        // Fallback: método antiguo (por si cambia la estructura)
        if (videos.isEmpty()) {
    ...
    /**
     * Filtra la lista de videos según las preferencias del usuario:
     *  - preferred_server: nombre del servidor (Voe, Mp4upload, Uqload, StreamWish)
     *  - preferred_quality: "any", "480", "720", "1080"
     *
     * Lógica:
     *  1) Intentar filtrar por servidor (si hay coincidencias, usar ese subconjunto).
     *  2) Luego filtrar por calidad (si se seleccionó distinta de "any").
     *  Si un filtro no produce coincidencias, se mantiene la lista anterior para evitar devolver vacío.
     */
    private fun filterByPreferences(videos: List<Video>): List<Video> {
        val preferredServer = preferences.getString("preferred_server", "Voe")
        val preferredQuality = preferences.getString("preferred_quality", "any")
