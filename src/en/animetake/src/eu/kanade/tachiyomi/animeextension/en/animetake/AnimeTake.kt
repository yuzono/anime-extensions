...
class AnimeTake :
    ...
    override suspend fun getPopularAnime(page: Int): AnimesPage {
        val response = client.newCall(GET("$baseUrl/animelist/popular", headers)).awaitSuccess()
    ...
    private suspend fun videosFromVidara(embedUrl: String): List<Video> {
        ...
        val response = client.newCall(
            POST(apiUrl, embedHeaders, VidaraStreamRequestDto(filecode).toJsonRequestBody()),
        ).awaitSuccess()
    ...
    private suspend fun resolveEmbed(redirectUrl: String): String? = runCatching {
        client.newCall(GET(redirectUrl, headers)).awaitSuccess().use { resp ->
