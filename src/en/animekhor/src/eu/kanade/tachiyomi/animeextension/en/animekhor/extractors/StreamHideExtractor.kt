...
import keiyoushi.utils.UrlUtils.fixUrl
...
class StreamHideExtractor(private val client: OkHttpClient, private val headers: Headers) {
    ...
    fun videosFromUrl(url: String, prefix: String = ""): List<Video> {
        val page = client.newCall(GET(url, headers = headers)).execute().use { it.body.string() }
        val unpacked = JsUnpacker.unpackAndCombine(page) ?: page
        val playlistUrl = unpacked.substringAfter("sources:")
            .substringAfter("file:\"")
            .substringBefore('"')

        val playlistData = client.newCall(GET(playlistUrl, headers = headers)).execute().use { it.body.string() }

        val subs = subtitleRegex.findAll(playlistData).mapNotNull {
            fixUrl(it.groupValues[2], playlistUrl)?.let { subUrl ->
        ...
        return playlistData.substringAfter(separator).split(separator).mapNotNull {
            ...
            fixUrl(urlPart, playlistUrl)?.let { videoUrl ->
